...
public class Parser {
	...
	public String convertWebLinksToString(WebLink[] links) {
		...
		for (WebLink wl : links) {
			...
			result += wl.getURLString();
		}
		return result;
	}
	
	public String convertStringArrayToString(String[] stringsToConcatenate) {
		String result = "";
		boolean isFirstLine = true;
		
		for (String s1 : stringsToConcatenate) {
			if (isFirstLine) {
				isFirstLine = false;
			} else {
				result += endOfLine;
	...
	public String convertWebImagesToString(WebImage[] images) {
		String result = "";
		boolean isFirstLine = true;
		
		for (WebImage i1 : images) {
			if (isFirstLine) {
				isFirstLine = false;
			} else {
				result += endOfLine;
